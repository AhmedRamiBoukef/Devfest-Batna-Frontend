...
const Login = () => {
  ...
  const handleClick = () => {
    ...
    console.log(data);
    axios.post("http://10.33.30.159:5000/hospital/login", 
        data
    )
      .then(function (response) {
        if (response.status===200){
          setUser(response.data.hospital);
          localStorage.setItem('hospital',response.data.hospital);
  ...
  return (
    <>
    {!user && <div className="bg-[#3CB79F] bg-opacity-30 lg:p-14 md:p-10 sm:p-6 p-4 ">
      <div className=" mb-[5%] flex justify-between bg-[#FFFF] mx-[5%] rounded-xl lg:p-5 md:p-4 sm:p-3 p-2 shadow-lg">
        <div className=" shadow-sm rounded-md grow lg:p-5 md:p-4 sm:p-3 p-2">
          ...
          <button onClick={handleClick} className="border-2 bg-[#3CB79F] hover:cursor-pointer font-bold text-white lg:text-[16px] md:text-[13px] sm:text-[10px] text-[7px] rounded-lg focus:ring-blue-500 focus:border-blue-500 w-fit mt-8 px-8 py-2">
            Login
          </button>
